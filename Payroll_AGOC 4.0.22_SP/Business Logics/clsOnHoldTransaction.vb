...
#Region "Populate Employee Details"
    Private Sub PopulateEmployeeDetails(ByVal aForm As SAPbouiCOM.Form)
        Try
            ...
            strQuery = "SELECT T0.[U_Z_EmpId],T0.""empID"", T0.""ExtEmpNo"" 'U_Z_ExtNo',T0.""firstName"" + isnull( T0.[middleName],'') + isnull(T0.[lastName],'') 'Name',  convert(varchar,T1.""U_Z_Month"") 'U_Z_Month',convert(varchar,T1.U_Z_Year) 'U_Z_Year',T1.[U_Z_Remarks] FROM OHEM T0 left outer Join  [dbo].[@Z_PAY20]  T1 on T1.U_Z_EmpId=T0.empID"
    ...
    Private Sub TransactionDetails(ByVal aform As SAPbouiCOM.Form)
        Try
            ...
            strQuery = "SELECT T0.[U_Z_EmpId1],T0.[Code], T0.[Name],T0.U_Z_ExtNo, T0.[U_Z_EmpId] ""U_Z_EmpId"", T0.U_Z_EmpName ,  Convert(Varchar,T0.[U_Z_Month]) 'U_Z_Month', Convert(varchar,T0.[U_Z_Year]) 'U_Z_Year', T0.[U_Z_Remarks] FROM [dbo].[@Z_PAY20]  T0"
