...
#Region "Populate Employee Details"
    Private Sub PopulateEmployeeDetails(ByVal aForm As SAPbouiCOM.Form)
        Try
            ...
            strQuery = "SELECT T0.""U_Z_EmpId"",T0.[empID],T0.ExtEmpNo 'U_Z_ExtNo', T0.[firstName] + isnull( T0.[middleName],'') + isnull(T0.[lastName],'') 'Name',  T1.[U_Z_TrnsCode], T1.U_Z_LeaveName,  T1.[U_Z_StartDate],  T1.[U_Z_NoofDays],T1.""U_Z_CashOut"", T1.[U_Z_Notes]  FROM OHEM T0 left outer Join  [dbo].[@Z_PAY_OLADJTRANS]  T1 on T1.U_Z_EMPID=T0.empID"
    ...
    Private Sub TransactionDetails(ByVal aform As SAPbouiCOM.Form)
        Try
            ...
            strQuery = "SELECT T0.[U_Z_EmpId1],T0.[Code],T0.U_Z_ExtNo, T0.[Name], T0.[U_Z_EMPID],T0.""U_Z_EMPNAME"", T0.[U_Z_TrnsCode], T0.U_Z_LeaveName, T0.[U_Z_StartDate], T0.[U_Z_NoofDays],T0.[U_Z_CashOut],T0.[U_Z_Notes] FROM [dbo].[@Z_PAY_OLADJTRANS]  T0"
